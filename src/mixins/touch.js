...
export default {
  methods: {
    ...
    initUserTouchEvent() {
      ...
      helper.bindEvent(el, constant.TOUCH_START, (e) => {
        e.preventDefault();
      ...
      helper.bindEvent(el, constant.TOUCH_MOVE, (e) => {
        e.preventDefault();
    ...
    setPageNow(offset) {
      ...
      // Set page number,
      // It should put DOM operation to asynchronous queue in order to prevent unfluency.
      setTimeout(() => {
    ...
    getStyle(index) {
      let css = '';
      const last = this.cards.length - 1 === index;
      const gapWidth = this.options.gapWidth || 0;
      if (gapWidth && !last) {
